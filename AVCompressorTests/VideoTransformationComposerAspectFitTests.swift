...
//  AVCompressorTests
//
//  Created by Sergey Kazakov on 26/12/2019.
//  Copyright © 2019 kazaimazai. All rights reserved.
//

import Foundation

class VideoTransformationComposerAspectFitOnlyResizeTests: ParametrizedTestCase {
  func methodToTestWithParameters(originalSize: CGSize, targetSize: CGSize) {
    
    let resizeContentMode = ResizeContentMode.aspectFit(targetSize)
    ...
    //when aspect fit, crop offset should be always <= 0
    XCTAssertLessThanOrEqual(resultOffsetX, 0)
    XCTAssertLessThanOrEqual(resultOffsetY, 0)
  }
...
class VideoTransformationComposerAspectFitOnlyCropTests: ParametrizedTestCase {
  func methodToTestWithParameters(originalSize: CGSize, crop: Crop) {
    
    //only crop, no additional resize to fit occur
...
class VideoTransformationComposerAspectFitResizeWithCropTests: ParametrizedTestCase {
